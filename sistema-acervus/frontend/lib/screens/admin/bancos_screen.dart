...
import '../../models/_core/banco.dart' as banco;
import '../../services/_financeiro/banco_service.dart' as bancoService;
...
class _BancosScreenState extends State<BancosScreen>
  ...
  Future<void> _loadBancos({bool showLoading = true}) async {
    if (!mounted) return;
    ...
    try {
      final result = await bancoService.BancoService.listarBancos(
        page: _currentPage,
        limit: _pageSize,
      ...
      if (mounted) {
        setState(() {
          _bancos = result['bancos'];
          _pagination = result['pagination'];

          if (_pagination == null || _pagination!.isEmpty) {
            final totalItems = _bancos.length;
  ...
  Future<void> _loadEstatisticas() async {
    try {
      final stats =
          await bancoService.BancoService.getCachedEstatisticasGerais();
      setState(() {
        _estatisticas = stats;
      });
    } catch (e) {
  ...
  void _performSearch() async {
    ...
    try {
      ...
      final result =
          await bancoService.BancoService.buscarBanco(_currentSearch);

      if (mounted) {
        setState(() {
          _bancos = result ?? <banco.Banco>[];
          _pagination = {
            'currentPage': 1,
            'totalPages': 1,
            'total': _bancos.length,
            'hasNextPage': false,
            'hasPrevPage': false,
          };
        });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Cursos e Bancos'),
        actions: [
          IconButton(
  ...
  Widget _buildListaTab() {
    return Column(
      children: [
        ...
        Expanded(child: buscarBancosList()),
            ...
            children: [
              Expanded(
                child: _buildStatCard(
                  'Total de Status',
                  (_pagination?['total'] ?? _bancos.length).toString(),
                  Icons.description,
              ...
              Expanded(
                child: _buildStatCard(
                  'Ativos',
                  (_bancos.where((s) => s.ativo).length).toString(),
                  Icons.check_circle,
              ...
              Expanded(
                child: _buildStatCard(
                  'Padrão',
                  (_bancos.where((s) => s.isDefault).length).toString(),
                  Icons.star,
  ...
  Widget buscarBancosList() {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        final bancoBanco = _bancos[index];
        return _buildBancoCard(bancoBanco, index);
      },
    );
  }

  Widget _buildBancoCard(banco.Banco bancoBanco, int index) {
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Container(
                  ...
                  child: Center(
                    child: Text(
                      bancoBanco.ordem?.toString() ?? '${index + 1}',
                      style: const TextStyle(
                        fontSize: 12,
                        fontWeight: FontWeight.bold,
                        color: Color(0xFF2E7D32),
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      if (bancoBanco.descricao.isNotEmpty)
                        Text(
                          bancoBanco.descricao,
                          style: TextStyle(
                            color: Colors.grey[600],
                            fontSize: 12,
                          ),
                ...
                PopupMenuButton<String>(
                  onSelected: (value) => _handleMenuAction(value, bancoBanco),
                  itemBuilder: (context) => [
                    const PopupMenuItem(
                      value: 'editar',
                      child: Row(
                    ...
                    PopupMenuItem(
                      value: bancoBanco.ativo ? 'desativar' : 'ativar',
                      child: Row(
                        children: [
                          Icon(
                            bancoBanco.ativo
                            ...
                            size: 18,
                            color:
                                bancoBanco.ativo ? Colors.orange : Colors.green,
                          ),
                          const SizedBox(width: 8),
                          Text(
                            bancoBanco.ativo ? 'Desativar' : 'Ativar',
                            style: TextStyle(
                              color: bancoBanco.ativo
                                  ? Colors.orange
                                  : Colors.green,
                            ),
                          ),
            ...
            //         'Criado em',
            //         bancoBanco.createdAt != null
            //             ? AppUtils.formatDate(bancoBanco.createdAt!)
            //             : 'N/A',
              ...
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                      color: bancoBanco.ativo ? Colors.green : Colors.grey,
                    ),
                  ),
                  child: Text(
                    bancoBanco.ativo ? 'ATIVO' : 'INATIVO',
                    style: TextStyle(
                      fontSize: 10,
                      fontWeight: FontWeight.bold,
                      color: bancoBanco.ativo ? Colors.green : Colors.grey,
  ...
  Widget _buildPaginationControls() {
    if (_bancos.isEmpty && !_isLoading && !_isLoadingPage) {
      return const SizedBox.shrink();
    }

    final totalPages = _pagination?['totalPages'] ??
        ((_bancos.isNotEmpty)
            ? ((_bancos.length / _pageSize)
                .ceil()
                .clamp(1, double.infinity)
                .toInt())
            : 1);
    final currentPage = _pagination?['currentPage'] ?? _currentPage;
    final total = _pagination?['total'] ?? _bancos.length;
    final hasNextPage =
  ...
  void _handleMenuAction(String action, banco.Banco bancoBanco) {
    switch (action) {
      case 'editar':
        _editarBanco(bancoBanco);
        break;
      case 'ativar':
        _ativarBanco(bancoBanco);
        break;
      case 'desativar':
        _desativarBanco(bancoBanco);
        break;
      case 'excluir':
        _confirmarExclusao(bancoBanco);
        break;
    }
  }

  void _editarBanco(banco.Banco bancoBanco) {
    setState(() {
      _statusEditando = bancoBanco;
      _nomeController.text = bancoBanco.nome;
      //_descricaoController.text = bancoBanco.descricao;
      _ordemController.text = bancoBanco.ordem?.toString() ?? '';
      _corSelecionada = bancoBanco.cor;
      _ativo = bancoBanco.ativo;
      _isDefault = bancoBanco.isDefault;
      _showForm = true;
    });
  }

  Future<void> _ativarBanco(banco.Banco bancoBanco) async {
    try {
      final success =
          await bancoService.BancoService.ativarBanco(bancoBanco.id!);
      if (success) {
  ...
  Future<void> _desativarBanco(banco.Banco bancoBanco) async {
    try {
      final success =
          await bancoService.BancoService.desativarBanco(bancoBanco.id!);
      if (success) {
  ...
  Future<void> _confirmarExclusao(banco.Banco bancoBanco) async {
    final confirm = await AppUtils.showConfirmDialog(
      context,
      title: 'Excluir Item',
      content:
          'Tem certeza que deseja excluir "${bancoBanco.nome}"?\n\nEsta ação não pode ser desfeita e pode afetar cursos que utilizam este status.',
      confirmText: 'Excluir',
    );
    ...
    if (confirm) {
      try {
        final success =
            await bancoService.BancoService.deletarBanco(bancoBanco.id!);
        if (success) {
  ...
  Future<void> _salvarStatus() async {
    ...
    try {
      ...
      if (_statusEditando == null) {
        success = await bancoService.BancoService.criarBanco(dados);
      } else {
        success = await bancoService.BancoService.atualizarBanco(
  ...
  Future<void> _exportarDados() async {
    try {
      await bancoService.BancoService.exportarCSV(
        ativo: _filtroAtivo,
        isDefault: _filtroDefault,
      );
