...
import 'package:sistema_estagio/models/_organizacoes/empresa/setor.dart'
    as setor;
import 'package:sistema_estagio/services/_organizacoes/empresa/setor_service.dart'
    as setorService;
...
class _SetoresScreenState extends State<SetoresScreen>
  ...
  Future<void> _loadSetores({bool showLoading = true}) async {
    if (!mounted) return;
    ...
    try {
      final result = await setorService.SetorService.listarSetores(
        page: _currentPage,
        limit: _pageSize,
      ...
      if (mounted) {
        setState(() {
          _setores = result['setores'];
          _pagination = result['pagination'];

          if (_pagination == null || _pagination!.isEmpty) {
            final totalItems = _setores.length;
  ...
  Future<void> _loadEstatisticas() async {
    try {
      final stats =
          await setorService.SetorService.getCachedEstatisticasGerais();
      setState(() {
        _estatisticas = stats;
      });
    } catch (e) {
  ...
  void _performSearch() async {
    ...
    try {
      ...
      final result =
          await setorService.SetorService.buscarSetor(_currentSearch);

      if (mounted) {
        setState(() {
          _setores = result ?? <setor.Setor>[];
          _pagination = {
            'currentPage': 1,
            'totalPages': 1,
            'total': _setores.length,
            'hasNextPage': false,
            'hasPrevPage': false,
          };
        });
  ...
  Widget _buildListaTab() {
    return Column(
      children: [
        ...
        Expanded(child: buscarSetoresList()),
            ...
            children: [
              Expanded(
                child: _buildStatCard(
                  'Total de Status',
                  (_pagination?['total'] ?? _setores.length).toString(),
                  Icons.description,
              ...
              Expanded(
                child: _buildStatCard(
                  'Ativos',
                  (_setores.where((s) => s.ativo).length).toString(),
                  Icons.check_circle,
              ...
              Expanded(
                child: _buildStatCard(
                  'Padrão',
                  (_setores.where((s) => s.isDefault).length).toString(),
                  Icons.star,
  ...
  Widget buscarSetoresList() {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return _buildSetorCard(setor, index);
      },
    );
  }

  Widget _buildSetorCard(setor.Setor setor, int index) {
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                ...
                PopupMenuButton<String>(
                  onSelected: (value) => _handleMenuAction(value, setor),
                  itemBuilder: (context) => [
                    const PopupMenuItem(
                      value: 'editar',
                      child: Row(
                    ...
                    PopupMenuItem(
                      value: setor.ativo ? 'desativar' : 'ativar',
                      child: Row(
                        children: [
                          Icon(
                            setor.ativo
                                ...
                                : Icons.visibility,
                            size: 18,
                            color: setor.ativo ? Colors.orange : Colors.green,
                          ),
                          const SizedBox(width: 8),
                          Text(
            ...
            //         'Criado em',
            //         setor.createdAt != null
            //             ? AppUtils.formatDate(setor.createdAt!)
            //             : 'N/A',
              ...
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                      color: setor.ativo ? Colors.green : Colors.grey,
                    ),
                  ),
                  child: Text(
  ...
  Widget _buildPaginationControls() {
    if (_setores.isEmpty && !_isLoading && !_isLoadingPage) {
      return const SizedBox.shrink();
    }

    final totalPages = _pagination?['totalPages'] ??
        ((_setores.isNotEmpty)
            ? ((_setores.length / _pageSize)
                .ceil()
                .clamp(1, double.infinity)
                .toInt())
            : 1);
    final currentPage = _pagination?['currentPage'] ?? _currentPage;
    final total = _pagination?['total'] ?? _setores.length;
    final hasNextPage =
  ...
  void _goToPage(int page) {
    if (page != _currentPage && page >= 1) {
      ...
      _loadSetores(showLoading: false);
    }
  }
  ...
  void _handleMenuAction(String action, setor.Setor setor) {
    switch (action) {
      case 'editar':
        _editarSetor(setor);
        break;
      case 'ativar':
        ...
        break;
      case 'desativar':
        _desativarSetor(setor);
        break;
      case 'excluir':
        _confirmarExclusao(setor);
        break;
    }
  }

  void _editarSetor(setor.Setor setor) {
    setState(() {
      _statusEditando = setor;
      _nomeController.text = setor.nome;
      //_descricaoController.text = setor.descricao;
      //_ordemController.text = setor.ordem?.toString() ?? '';
      //_corSelecionada = setor.cor;
      _ativo = setor.ativo;
      //_isDefault = setor.isDefault;
      _showForm = true;
    });
  }

  Future<void> _ativarSetor(setor.Setor setor) async {
    try {
      final success = await setorService.SetorService.ativarSetor(setor.id);
      if (success) {
  ...
  Future<void> _desativarSetor(setor.Setor setor) async {
    try {
      final success = await setorService.SetorService.desativarSetor(setor.id);
      if (success) {
  ...
  Future<void> _confirmarExclusao(setor.Setor setor) async {
    ...
    if (confirm) {
      try {
        final success = await setorService.SetorService.deletarSetor(setor.id);
        if (success) {
  ...
  Future<void> _salvarStatus() async {
    ...
    try {
      ...
      if (_statusEditando == null) {
        success = await setorService.SetorService.criarSetor(dados);
      } else {
        success = await setorService.SetorService.atualizarSetor(
  ...
  Future<void> _exportarDados() async {
    try {
      await setorService.SetorService.exportarSetoresCSV(
        ativo: _filtroAtivo,
        isDefault: _filtroDefault,
      );
