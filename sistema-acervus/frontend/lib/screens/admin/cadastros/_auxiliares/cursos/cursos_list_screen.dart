...
class _CursosScreenState extends State<CursosScreen>
  ...
  Future<void> _loadCursos({bool showLoading = true}) async {
    ...
    try {
      final result = await CursoService.listarCursos(
        page: _currentPage,
        limit: _pageSize,
  ...
  Future<void> _loadEstatisticas() async {
    try {
      final stats = await CursoService.getCachedEstatisticasGerais();
      setState(() {
        _estatisticas = stats;
      });
    } catch (e) {
  ...
  void _performSearch() async {
    ...
    try {
      ...
      final result = await CursoService.buscarCurso(_currentSearch);

      if (mounted) {
        setState(() {
          _cursos = result ?? <Curso>[];
          _pagination = {
            'currentPage': 1,
            'totalPages': 1,
  ...
  Widget _buildListaTab() {
    return Column(
      children: [
        ...
        Expanded(child: buscarCursosList()),
          ...
          Row(
            children: [
              ...
              Expanded(
                child: _buildStatCard(
                  'Padrão',
                  (_cursos.where((s) => s.isDefault).length).toString(),
                  Icons.star,
  ...
  Widget buscarCursosList() {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        final nivelFormacao = _cursos[index];
        return _buildCursoCard(nivelFormacao, index);
      },
    );
  }

  Widget _buildCursoCard(Curso nivelFormacao, int index) {
    return Card(
  ...
  void _handleMenuAction(String action, Curso nivelFormacao) {
    switch (action) {
      case 'editar':
        _editarCurso(nivelFormacao);
        break;
      case 'ativar':
        _ativarCurso(nivelFormacao);
        break;
      case 'desativar':
        _desativarCurso(nivelFormacao);
        break;
      case 'excluir':
  ...
  void _editarCurso(Curso nivelFormacao) {
    setState(() {
  ...
  Future<void> _ativarCurso(Curso nivelFormacao) async {
    try {
      final success = await CursoService.ativarCurso(nivelFormacao.id!);
      if (success) {
  ...
  Future<void> _desativarCurso(Curso nivelFormacao) async {
    try {
      final success = await CursoService.desativarCurso(nivelFormacao.id!);
      if (success) {
  ...
  Future<void> _confirmarExclusao(Curso nivelFormacao) async {
    ...
    if (confirm) {
      try {
        final success = await CursoService.deletarCurso(nivelFormacao.id!);
        if (success) {
  ...
  Future<void> _salvarStatus() async {
    ...
    try {
      ...
      if (_statusEditando == null) {
        success = await CursoService.criarCurso(dados);
      } else {
        success =
            await CursoService.atualizarCurso(_statusEditando!.id!, dados);
      }
  ...
  Future<void> _exportarDados() async {
    try {
      await CursoService.exportarCSV(
        ativo: _filtroAtivo,
        isDefault: _filtroDefault,
      );
