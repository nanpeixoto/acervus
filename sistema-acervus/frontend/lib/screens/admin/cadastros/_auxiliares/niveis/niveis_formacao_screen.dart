...
class NiveisFormacaoScreen extends StatefulWidget {
  const NiveisFormacaoScreen({super.key});

  @override
  State<NiveisFormacaoScreen> createState() => _NiveisFormacaoScreenState();
}

class _NiveisFormacaoScreenState extends State<NiveisFormacaoScreen>
    with TickerProviderStateMixin {
  final _searchController = TextEditingController();

  List<nivel.NivelFormacao> _niveisFormacao = [];
  ...
  nivel.NivelFormacao? _statusEditando;
  ...
  Future<void> _loadNiveisFormacao({bool showLoading = true}) async {
    if (!mounted) return;
    ...
    try {
      final result =
          await nivelService.NivelFormacaoService.listarNiveisFormacao(
        page: _currentPage,
        limit: _pageSize,
      ...
      if (mounted) {
        setState(() {
          _niveisFormacao = result['niveisFormacao'];
          _pagination = result['pagination'];

          if (_pagination == null || _pagination!.isEmpty) {
            final totalItems = _niveisFormacao.length;
  ...
  Future<void> _loadEstatisticas() async {
    try {
      final stats =
          await nivelService.NivelFormacaoService.getCachedEstatisticasGerais();
      setState(() {
        _estatisticas = stats;
      });
    } catch (e) {
  ...
  void _performSearch() async {
    ...
    try {
      if (_currentSearch.isEmpty) {
        await _loadNiveisFormacao();
        return;
      }

      final result =
          await nivelService.NivelFormacaoService.buscarNivelFormacao(
              _currentSearch);

      if (mounted) {
        setState(() {
          _niveisFormacao = result ?? <nivel.NivelFormacao>[];
          _pagination = {
            'currentPage': 1,
            'totalPages': 1,
            'total': _niveisFormacao.length,
            'hasNextPage': false,
            'hasPrevPage': false,
          };
        });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Nível de Formação'),
        actions: [
          IconButton(
          ...
          IconButton(
            ...
            tooltip: 'Adicionar Nível de Formação',
          ),
        ],
        // bottom: TabBar(
  ...
  Widget _buildListaTab() {
    return Column(
      children: [
        ...
        Expanded(child: buscarNiveisFormacaoList()),
            ...
            children: [
              Expanded(
                child: _buildStatCard(
                  'Total de Status',
                  (_pagination?['total'] ?? _niveisFormacao.length).toString(),
                  Icons.description,
              ...
              Expanded(
                child: _buildStatCard(
                  'Ativos',
                  (_niveisFormacao.where((s) => s.ativo).length).toString(),
                  Icons.check_circle,
              ...
              Expanded(
                child: _buildStatCard(
                  'Padrão',
                  (_niveisFormacao.where((s) => s.isDefault).length).toString(),
                  Icons.star,
  ...
  Widget buscarNiveisFormacaoList() {
    if (_isLoadingPage) {
      return const Center(
        child: Column(
    ...
    return ListView.builder(
      padding: const EdgeInsets.all(16),
      itemCount: _niveisFormacao.length,
      itemBuilder: (context, index) {
        final nivelFormacao = _niveisFormacao[index];
        return _buildNivelFormacaoCard(nivelFormacao, index);
      },
    );
  }

  Widget _buildNivelFormacaoCard(nivel.NivelFormacao nivelFormacao, int index) {
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Container(
                  ...
                  child: Center(
                    child: Text(
                      nivelFormacao.ordem?.toString() ?? '${index + 1}',
                      style: const TextStyle(
                        fontSize: 12,
                        fontWeight: FontWeight.bold,
                        color: Color(0xFF2E7D32),
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      if (nivelFormacao.descricao.isNotEmpty)
                        Text(
                          nivelFormacao.descricao,
                          style: TextStyle(
                            color: Colors.grey[600],
                            fontSize: 12,
                          ),
                ...
                PopupMenuButton<String>(
                  onSelected: (value) =>
                      _handleMenuAction(value, nivelFormacao),
                  itemBuilder: (context) => [
                    const PopupMenuItem(
                      value: 'editar',
                      child: Row(
                    ...
                    PopupMenuItem(
                      value: nivelFormacao.ativo ? 'desativar' : 'ativar',
                      child: Row(
                        children: [
                          Icon(
                            nivelFormacao.ativo
                                ...
                                : Icons.visibility,
                            size: 18,
                            color: nivelFormacao.ativo
                                ? Colors.orange
                                : Colors.green,
                          ),
                          const SizedBox(width: 8),
                          Text(
                            nivelFormacao.ativo ? 'Desativar' : 'Ativar',
                            style: TextStyle(
                              color: nivelFormacao.ativo
                                  ? Colors.orange
                                  : Colors.green,
            ...
            //         'Criado em',
            //         nivelFormacao.createdAt != null
            //             ? AppUtils.formatDate(nivelFormacao.createdAt!)
            //             : 'N/A',
            ...
            //       child: _buildInfoChip(
            //         'Usos',
            //         nivelFormacao.totalUsos.toString(),
            //         Icons.assignment,
            //       ),
            //     ),
            //   ],
              ...
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                      color: nivelFormacao.ativo ? Colors.green : Colors.grey,
                    ),
                  ),
                  child: Text(
                    nivelFormacao.ativo ? 'ATIVO' : 'INATIVO',
                    style: TextStyle(
                      fontSize: 10,
                      fontWeight: FontWeight.bold,
                      color: nivelFormacao.ativo ? Colors.green : Colors.grey,
  ...
  Widget _buildPaginationControls() {
    if (_niveisFormacao.isEmpty && !_isLoading && !_isLoadingPage) {
      return const SizedBox.shrink();
    }

    final totalPages = _pagination?['totalPages'] ??
        ((_niveisFormacao.isNotEmpty)
            ? ((_niveisFormacao.length / _pageSize)
                .ceil()
                .clamp(1, double.infinity)
                .toInt())
            : 1);
    final currentPage = _pagination?['currentPage'] ?? _currentPage;
    final total = _pagination?['total'] ?? _niveisFormacao.length;
    final hasNextPage =
  ...
  void _goToPage(int page) {
    if (page != _currentPage && page >= 1) {
      ...
      _loadNiveisFormacao(showLoading: false);
    }
  }
  ...
  void _handleMenuAction(String action, nivel.NivelFormacao nivelFormacao) {
    switch (action) {
      case 'editar':
        _editarNivelFormacao(nivelFormacao);
        break;
      case 'ativar':
        _ativarNivelFormacao(nivelFormacao);
        break;
      case 'desativar':
        _desativarNivelFormacao(nivelFormacao);
        break;
      case 'excluir':
        _confirmarExclusao(nivelFormacao);
        break;
    }
  }

  void _editarNivelFormacao(nivel.NivelFormacao nivelFormacao) {
    setState(() {
      _statusEditando = nivelFormacao;
      _nomeController.text = nivelFormacao.nome;
      //_descricaoController.text = nivelFormacao.descricao;
      _ordemController.text = nivelFormacao.ordem?.toString() ?? '';
      _corSelecionada = nivelFormacao.cor;
      _ativo = nivelFormacao.ativo;
      _isDefault = nivelFormacao.isDefault;
      _showForm = true;
    });
  }

  Future<void> _ativarNivelFormacao(nivel.NivelFormacao nivelFormacao) async {
    try {
      final success =
          await nivelService.NivelFormacaoService.ativarNivelFormacao(
              nivelFormacao.id!);
      if (success) {
  ...
  Future<void> _desativarNivelFormacao(
      nivel.NivelFormacao nivelFormacao) async {
    try {
      final success =
          await nivelService.NivelFormacaoService.desativarNivelFormacao(
              nivelFormacao.id!);
      if (success) {
  ...
  Future<void> _confirmarExclusao(nivel.NivelFormacao nivelFormacao) async {
    final confirm = await AppUtils.showConfirmDialog(
      context,
      title: 'Excluir Item',
      content:
          'Tem certeza que deseja excluir "${nivelFormacao.nome}"?\n\nEsta ação não pode ser desfeita e pode afetar cursos que utilizam este status.',
      confirmText: 'Excluir',
    );
    ...
    if (confirm) {
      try {
        final success =
            await nivelService.NivelFormacaoService.deletarNivelFormacao(
                nivelFormacao.id!);
        if (success) {
  ...
  Future<void> _salvarStatus() async {
    ...
    try {
      ...
      if (_statusEditando == null) {
        success =
            await nivelService.NivelFormacaoService.criarNivelFormacao(dados);
      } else {
        success =
            await nivelService.NivelFormacaoService.atualizarNivelFormacao(
  ...
  Future<void> _exportarDados() async {
    try {
      await nivelService.NivelFormacaoService.exportarCSV(
        ativo: _filtroAtivo,
        isDefault: _filtroDefault,
      );
