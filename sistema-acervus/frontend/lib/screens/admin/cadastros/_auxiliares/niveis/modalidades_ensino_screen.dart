...
class ModalidadesEnsinoScreen extends StatefulWidget {
  const ModalidadesEnsinoScreen({super.key});

  @override
  State<ModalidadesEnsinoScreen> createState() =>
      _ModalidadesEnsinoScreenState();
}

class _ModalidadesEnsinoScreenState extends State<ModalidadesEnsinoScreen>
    with TickerProviderStateMixin {
  final _searchController = TextEditingController();

  List<modalidade.Modalidade> _modalidades = [];
  ...
  modalidade.Modalidade? _statusEditando;
  ...
  Future<void> _loadModalidades({bool showLoading = true}) async {
    if (!mounted) return;
    ...
    try {
      final result =
          await modalidadeService.ModalidadeService.listarModalidades(
        page: _currentPage,
        limit: _pageSize,
      ...
      if (mounted) {
        setState(() {
          _modalidades = result['modalidades'];
          _pagination = result['pagination'];

          if (_pagination == null || _pagination!.isEmpty) {
            final totalItems = _modalidades.length;
  ...
  Future<void> _loadEstatisticas() async {
    try {
      final stats = await modalidadeService.ModalidadeService
  ...
  void _performSearch() async {
    ...
    try {
      if (_currentSearch.isEmpty) {
        await _loadModalidades();
        return;
      }

      final result = await modalidadeService.ModalidadeService.buscarModalidade(
          _currentSearch);

      if (mounted) {
        setState(() {
          _modalidades = result ?? <modalidade.Modalidade>[];
          _pagination = {
            'currentPage': 1,
            'totalPages': 1,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Modalidade de Ensino'),
        actions: [
          IconButton(
          ...
          IconButton(
            ...
            tooltip: 'Adicionar Modalidade de Ensino',
          ),
        ],
        // bottom: TabBar(
  ...
  Widget _buildListaTab() {
    return Column(
      children: [
        ...
        Expanded(child: _buildModalidadesList()),
            ...
            children: [
              Expanded(
                child: _buildStatCard(
                  'Total de Status',
                  (_pagination?['total'] ?? _modalidades.length).toString(),
                  Icons.description,
              ...
              Expanded(
                child: _buildStatCard(
                  'Ativos',
                  (_modalidades.where((s) => s.ativo).length).toString(),
                  Icons.check_circle,
              ...
              Expanded(
                child: _buildStatCard(
                  'Padrão',
                  (_modalidades.where((s) => s.isDefault).length).toString(),
                  Icons.star,
  ...
  Widget _buildModalidadesList() {
          ...
          children: [
            ...
            if (_currentSearch.isNotEmpty)
              ...
              ElevatedButton(
                ...
                child: const Text('Adicionar Primeira Modalidade de Ensino'),
              ),
          ],
        ),
      );
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return _buildModalidadeCard(modalidade, index);
      },
    );
  }

  Widget _buildModalidadeCard(modalidade.Modalidade modalidade, int index) {
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Container(
                  ...
                  child: Center(
                    child: Text(
                      modalidade.ordem?.toString() ?? '${index + 1}',
                      style: const TextStyle(
                        fontSize: 12,
                        fontWeight: FontWeight.bold,
                        color: Color(0xFF2E7D32),
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      if (modalidade.descricao.isNotEmpty)
                        Text(
                ...
                PopupMenuButton<String>(
                  onSelected: (value) => _handleMenuAction(value, modalidade),
                  itemBuilder: (context) => [
                    const PopupMenuItem(
                      value: 'editar',
                      child: Row(
                    ...
                    PopupMenuItem(
                      value: modalidade.ativo ? 'desativar' : 'ativar',
                      child: Row(
                        children: [
                          Icon(
                            modalidade.ativo
                            ...
                            size: 18,
                            color:
                                modalidade.ativo ? Colors.orange : Colors.green,
                          ),
                          const SizedBox(width: 8),
                          Text(
                            modalidade.ativo ? 'Desativar' : 'Ativar',
                            style: TextStyle(
                              color: modalidade.ativo
                                  ? Colors.orange
                                  : Colors.green,
                            ),
                          ),
            ...
            //         'Criado em',
            //         modalidade.createdAt != null
            //             ? AppUtils.formatDate(modalidade.createdAt!)
            //             : 'N/A',
              ...
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                      color: modalidade.ativo ? Colors.green : Colors.grey,
                    ),
                  ),
                  child: Text(
  ...
  Widget _buildPaginationControls() {
    if (_modalidades.isEmpty && !_isLoading && !_isLoadingPage) {
      return const SizedBox.shrink();
    }

    final totalPages = _pagination?['totalPages'] ??
        ((_modalidades.isNotEmpty)
            ? ((_modalidades.length / _pageSize)
                .ceil()
                .clamp(1, double.infinity)
                .toInt())
            : 1);
    final currentPage = _pagination?['currentPage'] ?? _currentPage;
    final total = _pagination?['total'] ?? _modalidades.length;
    final hasNextPage =
  ...
  void _goToPage(int page) {
    if (page != _currentPage && page >= 1) {
      ...
      _loadModalidades(showLoading: false);
    }
  }
  ...
  void _handleMenuAction(String action, modalidade.Modalidade modalidade) {
    switch (action) {
      case 'editar':
        _editarModalidade(modalidade);
        break;
      case 'ativar':
        _ativarModalidade(modalidade);
        break;
      case 'desativar':
        _desativarModalidade(modalidade);
        break;
      case 'excluir':
        _confirmarExclusao(modalidade);
        break;
    }
  }

  void _editarModalidade(modalidade.Modalidade modalidade) {
    setState(() {
      _statusEditando = modalidade;
      _nomeController.text = modalidade.nome;
      //_descricaoController.text = modalidade.descricao;
      _ordemController.text = modalidade.ordem?.toString() ?? '';
      _corSelecionada = modalidade.cor;
      _ativo = modalidade.ativo;
      _isDefault = modalidade.isDefault;
      _showForm = true;
    });
  }

  Future<void> _ativarModalidade(modalidade.Modalidade modalidade) async {
    try {
      final success =
          await modalidadeService.ModalidadeService.ativarModalidade(
              modalidade.id!);
      if (success) {
  ...
  Future<void> _desativarModalidade(modalidade.Modalidade modalidade) async {
    try {
      final success =
          await modalidadeService.ModalidadeService.desativarModalidade(
              modalidade.id!);
      if (success) {
  ...
  Future<void> _confirmarExclusao(modalidade.Modalidade modalidade) async {
    final confirm = await AppUtils.showConfirmDialog(
      context,
      title: 'Excluir Item',
      content:
          'Tem certeza que deseja excluir "${modalidade.nome}"?\n\nEsta ação não pode ser desfeita e pode afetar cursos que utilizam este status.',
      confirmText: 'Excluir',
    );
    ...
    if (confirm) {
      try {
        final success =
            await modalidadeService.ModalidadeService.deletarModalidade(
                modalidade.id!);
        if (success) {
  ...
  Future<void> _salvarStatus() async {
    ...
    try {
      ...
      if (_statusEditando == null) {
        success =
            await modalidadeService.ModalidadeService.criarModalidade(dados);
      } else {
        success = await modalidadeService.ModalidadeService.atualizarModalidade(
  ...
  Future<void> _exportarDados() async {
    try {
      await modalidadeService.ModalidadeService.exportarModalidadesCSV(
        ativo: _filtroAtivo,
        isDefault: _filtroDefault,
      );
