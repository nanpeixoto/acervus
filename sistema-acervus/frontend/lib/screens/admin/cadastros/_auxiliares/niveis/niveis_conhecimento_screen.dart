...
class NiveisConhecimentoScreen extends StatefulWidget {
  const NiveisConhecimentoScreen({super.key});

  @override
  State<NiveisConhecimentoScreen> createState() =>
      _NiveisConhecimentoScreenState();
}

class _NiveisConhecimentoScreenState extends State<NiveisConhecimentoScreen>
    with TickerProviderStateMixin {
  final _searchController = TextEditingController();

  List<nivel.NivelConhecimento> _niveisConhecimento = [];
  ...
  nivel.NivelConhecimento? _statusEditando;
  ...
  void initState() {
    ...
    _loadNiveisConhecimento();
    //_loadEstatisticas();
  }

  Future<void> _loadNiveisConhecimento({bool showLoading = true}) async {
    if (!mounted) return;
    ...
    try {
      final result =
          await nivelService.NivelConhecimentoService.listarNiveisConhecimento(
        page: _currentPage,
        limit: _pageSize,
      ...
      if (mounted) {
        setState(() {
          _niveisConhecimento = result['niveisConhecimento'];
          _pagination = result['pagination'];

          if (_pagination == null || _pagination!.isEmpty) {
            final totalItems = _niveisConhecimento.length;
  ...
  Future<void> _loadEstatisticas() async {
    try {
      final stats = await nivelService.NivelConhecimentoService
  ...
  void _performSearch() async {
    ...
    try {
      if (_currentSearch.isEmpty) {
        await _loadNiveisConhecimento();
        return;
      }

      final result =
          await nivelService.NivelConhecimentoService.buscarNivelConhecimento(
              _currentSearch);

      if (mounted) {
        setState(() {
          _niveisConhecimento = result ?? <nivel.NivelConhecimento>[];
          _pagination = {
            'currentPage': 1,
            'totalPages': 1,
            'total': _niveisConhecimento.length,
            'hasNextPage': false,
            'hasPrevPage': false,
          };
        });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Nível de Conhecimento'),
        actions: [
          IconButton(
          ...
          IconButton(
            ...
            tooltip: 'Adicionar Nível de Conhecimento',
          ),
        ],
        // bottom: TabBar(
  ...
  Widget _buildListaTab() {
    return Column(
      children: [
        ...
        Expanded(child: buscarNiveisConhecimentoList()),
            ...
            children: [
              Expanded(
                child: _buildStatCard(
                  'Total de Status',
                  (_pagination?['total'] ?? _niveisConhecimento.length)
                      .toString(),
                  Icons.description,
              ...
              Expanded(
                child: _buildStatCard(
                  'Ativos',
                  (_niveisConhecimento.where((s) => s.ativo).length).toString(),
                  Icons.check_circle,
              ...
              Expanded(
                child: _buildStatCard(
                  'Padrão',
                  (_niveisConhecimento.where((s) => s.isDefault).length)
                      .toString(),
                  Icons.star,
  ...
  Widget buscarNiveisConhecimentoList() {
    if (_isLoadingPage) {
      return const Center(
        child: Column(
    ...
    if (_niveisConhecimento.isEmpty) {
      return Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
    ...
    return ListView.builder(
      padding: const EdgeInsets.all(16),
      itemCount: _niveisConhecimento.length,
      itemBuilder: (context, index) {
        final nivelConhecimento = _niveisConhecimento[index];
        return _buildNivelConhecimentoCard(nivelConhecimento, index);
      },
    );
  }

  Widget _buildNivelConhecimentoCard(
      nivel.NivelConhecimento nivelConhecimento, int index) {
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Container(
                  ...
                  child: Center(
                    child: Text(
                      nivelConhecimento.ordem?.toString() ?? '${index + 1}',
                      style: const TextStyle(
                        fontSize: 12,
                        fontWeight: FontWeight.bold,
                        color: Color(0xFF2E7D32),
                  ...
                  child: Column(
                    ...
                    children: [
                      Row(
                        children: [
                          Text(
                            nivelConhecimento.nome,
                            style: const TextStyle(
                              fontWeight: FontWeight.w600,
                              fontSize: 16,
                            ),
                          ),
                          if (nivelConhecimento.isDefault) ...[
                            const SizedBox(width: 8),
                            Container(
                      ...
                      if (nivelConhecimento.descricao.isNotEmpty)
                        Text(
                          nivelConhecimento.descricao,
                          style: TextStyle(
                            color: Colors.grey[600],
                            fontSize: 12,
                          ),
                ...
                PopupMenuButton<String>(
                  onSelected: (value) =>
                      _handleMenuAction(value, nivelConhecimento),
                  itemBuilder: (context) => [
                    const PopupMenuItem(
                      value: 'editar',
                      child: Row(
                    ...
                    PopupMenuItem(
                      value: nivelConhecimento.ativo ? 'desativar' : 'ativar',
                      child: Row(
                        children: [
                          Icon(
                            nivelConhecimento.ativo
                                ? Icons.visibility_off
                                : Icons.visibility,
                            size: 18,
                            color: nivelConhecimento.ativo
                                ? Colors.orange
                                : Colors.green,
                          ),
                          const SizedBox(width: 8),
                          Text(
                            nivelConhecimento.ativo ? 'Desativar' : 'Ativar',
                            style: TextStyle(
                              color: nivelConhecimento.ativo
                                  ? Colors.orange
                                  : Colors.green,
            ...
            //         'Criado em',
            //         nivelConhecimento.createdAt != null
            //             ? AppUtils.formatDate(nivelConhecimento.createdAt!)
            //             : 'N/A',
            ...
            //       child: _buildInfoChip(
            //         'Usos',
            //         nivelConhecimento.totalUsos.toString(),
            //         Icons.assignment,
            //       ),
            //     ),
            //   ],
              ...
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                      color:
                          nivelConhecimento.ativo ? Colors.green : Colors.grey,
                    ),
                  ),
                  child: Text(
                    nivelConhecimento.ativo ? 'ATIVO' : 'INATIVO',
                    style: TextStyle(
                      fontSize: 10,
                      fontWeight: FontWeight.bold,
                      color:
  ...
  Widget _buildPaginationControls() {
    if (_niveisConhecimento.isEmpty && !_isLoading && !_isLoadingPage) {
      return const SizedBox.shrink();
    }

    final totalPages = _pagination?['totalPages'] ??
        ((_niveisConhecimento.isNotEmpty)
            ? ((_niveisConhecimento.length / _pageSize)
                .ceil()
                .clamp(1, double.infinity)
                .toInt())
            : 1);
    final currentPage = _pagination?['currentPage'] ?? _currentPage;
    final total = _pagination?['total'] ?? _niveisConhecimento.length;
    final hasNextPage =
  ...
  void _goToPage(int page) {
    if (page != _currentPage && page >= 1) {
      ...
      _loadNiveisConhecimento(showLoading: false);
    }
  }
  ...
  void _handleMenuAction(
      String action, nivel.NivelConhecimento nivelConhecimento) {
    switch (action) {
      case 'editar':
        _editarNivelConhecimento(nivelConhecimento);
        break;
      case 'ativar':
        _ativarNivelConhecimento(nivelConhecimento);
        break;
      case 'desativar':
        _desativarNivelConhecimento(nivelConhecimento);
        break;
      case 'excluir':
        _confirmarExclusao(nivelConhecimento);
        break;
    }
  }

  void _editarNivelConhecimento(nivel.NivelConhecimento nivelConhecimento) {
    setState(() {
      _statusEditando = nivelConhecimento;
      _nomeController.text = nivelConhecimento.nome;
      //_descricaoController.text = nivelConhecimento.descricao;
      _ordemController.text = nivelConhecimento.ordem?.toString() ?? '';
      _corSelecionada = nivelConhecimento.cor;
      _ativo = nivelConhecimento.ativo;
      _isDefault = nivelConhecimento.isDefault;
      _showForm = true;
    });
  }

  Future<void> _ativarNivelConhecimento(
      nivel.NivelConhecimento nivelConhecimento) async {
    try {
      final success =
          await nivelService.NivelConhecimentoService.ativarNivelConhecimento(
              nivelConhecimento.id!);
      if (success) {
  ...
  Future<void> _desativarNivelConhecimento(
      nivel.NivelConhecimento nivelConhecimento) async {
    try {
      final success = await nivelService.NivelConhecimentoService
          .desativarNivelConhecimento(nivelConhecimento.id!);
      if (success) {
  ...
  Future<void> _confirmarExclusao(
    ...
    if (confirm) {
      try {
        final success = await nivelService.NivelConhecimentoService
            .deletarNivelConhecimento(nivelConhecimento.id!);
        if (success) {
  ...
  Future<void> _salvarStatus() async {
    ...
    try {
      ...
      if (_statusEditando == null) {
        success =
            await nivelService.NivelConhecimentoService.criarNivelConhecimento(
                dados);
      } else {
        success = await nivelService.NivelConhecimentoService
            .atualizarNivelConhecimento(_statusEditando!.id!, dados);
      }
  ...
  Future<void> _exportarDados() async {
    try {
      await nivelService.NivelConhecimentoService.exportarCSV(
        ativo: _filtroAtivo,
        isDefault: _filtroDefault,
      );
