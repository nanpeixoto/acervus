...
class _TurnosScreenState extends State<TurnosScreen>
  ...
  Future<void> _loadTurnos({bool showLoading = true}) async {
    if (!mounted) return;
    ...
    try {
      final result = await TurnoService.listarTurnos(
        page: _currentPage,
        limit: _pageSize,
      ...
      if (mounted) {
        setState(() {
          ...
          if (_pagination == null || _pagination!.isEmpty) {
            final totalItems = _turnos.length;
  ...
  Future<void> _loadEstatisticas() async {
    try {
      final stats = await TurnoService.getCachedEstatisticasGerais();
      setState(() {
        _estatisticas = stats;
      });
    } catch (e) {
  ...
  void _performSearch() async {
    ...
    try {
      ...
      final result = await TurnoService.buscarTurno(_currentSearch);

      if (mounted) {
        setState(() {
          _turnos = result ?? <Turno>[];
          _pagination = {
            'currentPage': 1,
            'totalPages': 1,
            'total': _turnos.length,
            'hasNextPage': false,
            'hasPrevPage': false,
          };
        });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Gerenciar Turno'),
        actions: [
          IconButton(
  ...
  Widget _buildListaTab() {
    return Column(
      children: [
        ...
        Expanded(child: _buildTurnosList()),
      ...
      child: Column(
        children: [
          ...
          Row(
            children: [
              Expanded(
                child: _buildTurnCard(
                  'Total de Status',
                  (_pagination?['total'] ?? _turnos.length).toString(),
                  Icons.description,
              ...
              Expanded(
                child: _buildTurnCard(
                  'Ativos',
                  (_turnos.where((s) => s.ativo).length).toString(),
                  Icons.check_circle,
              ...
              Expanded(
                child: _buildTurnCard(
                  'Padrão',
                  (_turnos.where((s) => s.isDefault).length).toString(),
                  Icons.star,
          ...
          Row(
            children: [
              const Text('Turno por página: ', style: TextStyle(fontSize: 14)),
              DropdownButton<int>(
                value: _pageSize,
        ...
        child: Column(
          ...
          children: [
            Row(
              ...
              children: [
                Text(
                  _statusEditando == null ? 'Novo Turno' : 'Editar Turno',
            ...
            CustomTextField(
              ...
              maxLines: 1,
              validator: (value) =>
                  Validators.validateRequired(value, 'Nome do Turno'),
            ),
            const SizedBox(height: 16),
            CheckboxListTile(
  ...
  Widget _buildTurnosList() {
          ...
          children: [
            ...
            Text(
              _currentSearch.isNotEmpty
                  ? 'Nenhum turno encontrado para a busca "$_currentSearch"'
            ...
            if (_currentSearch.isNotEmpty)
              ...
              ElevatedButton(
                ...
                child: const Text('Adicionar Primeiro Turno'),
              ),
          ],
        ),
      );
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        final statusCurso = _turnos[index];
        return _buildTurnoCard(statusCurso, index);
      },
    );
  }

  Widget _buildTurnoCard(Turno statusCurso, int index) {
    return Card(
  ...
  Widget _buildPaginationControls() {
    if (_turnos.isEmpty && !_isLoading && !_isLoadingPage) {
      return const SizedBox.shrink();
    }

    final totalPages = _pagination?['totalPages'] ??
        ((_turnos.isNotEmpty)
            ? ((_turnos.length / _pageSize)
                .ceil()
                .clamp(1, double.infinity)
                .toInt())
            : 1);
    final currentPage = _pagination?['currentPage'] ?? _currentPage;
    final total = _pagination?['total'] ?? _turnos.length;
    final hasNextPage =
  ...
  void _handleMenuAction(String action, Turno statusCurso) {
    switch (action) {
      case 'editar':
        _editarTurno(statusCurso);
        break;
      case 'ativar':
        _ativarTurno(statusCurso);
        break;
      case 'desativar':
        _desativarTurno(statusCurso);
        break;
      case 'excluir':
  ...
  void _editarTurno(Turno statusCurso) {
    setState(() {
  ...
  Future<void> _ativarTurno(Turno statusCurso) async {
    try {
      final success = await TurnoService.ativarTurno(statusCurso.id!);
      if (success) {
  ...
  Future<void> _desativarTurno(Turno statusCurso) async {
    try {
      final success = await TurnoService.desativarTurno(statusCurso.id!);
      if (success) {
        AppUtils.showSuccessSnackBar(
  ...
  Future<void> _confirmarExclusao(Turno statusCurso) async {
    ...
    if (confirm) {
      try {
        final success = await TurnoService.deletarTurno(statusCurso.id!);
        if (success) {
          AppUtils.showSuccessSnackBar(
  ...
  Future<void> _salvarStatus() async {
    ...
    try {
      ...
      if (_statusEditando == null) {
        success = await TurnoService.criarTurno(dados);
      } else {
        success =
            await TurnoService.atualizarTurno(_statusEditando!.id!, dados);
      }
  ...
  Future<void> _exportarDados() async {
    try {
      final savedPath = await TurnoService.exportarTurnoCSV(
        ativo: _filtroAtivo,
        isDefault: _filtroDefault,
      );

      if (savedPath != null) {
        AppUtils.showSuccessSnackBar(context, 'CSV salvo em: $savedPath');
      } else {
        AppUtils.showSuccessSnackBar(
            context, 'Download iniciado no navegador.');
      }
    } catch (e) {
