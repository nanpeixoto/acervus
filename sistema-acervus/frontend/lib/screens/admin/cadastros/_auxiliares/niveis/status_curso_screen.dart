...
class StatusCursoScreen extends StatefulWidget {
  const StatusCursoScreen({super.key});

  @override
  State<StatusCursoScreen> createState() => _StatusCursoScreenState();
}

class _StatusCursoScreenState extends State<StatusCursoScreen>
    with TickerProviderStateMixin {
  final _searchController = TextEditingController();

  List<StatusCurso> _statusCursos = [];
  ...
  Future<void> _loadStatusCursos({bool showLoading = true}) async {
    if (!mounted) return;
    ...
    try {
      final result = await StatusCursoService.listarStatusCursos(
        page: _currentPage,
        limit: _pageSize,
      ...
      if (mounted) {
        setState(() {
          _statusCursos = result['statusCursos'];
          _pagination = result['pagination'];

          if (_pagination == null || _pagination!.isEmpty) {
            final totalItems = _statusCursos.length;
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Erro ao carregar status de cursos: $e'),
  ...
  Future<void> _loadEstatisticas() async {
    try {
      final stats = await StatusCursoService.getCachedEstatisticasGerais();
      setState(() {
        _estatisticas = stats;
      });
    } catch (e) {
  ...
  void _performSearch() async {
    ...
    try {
      if (_currentSearch.isEmpty) {
        await _loadStatusCursos();
        return;
      }

      final result = await StatusCursoService.buscarStatusCurso(_currentSearch);

      if (mounted) {
        setState(() {
          _statusCursos = result ?? <StatusCurso>[];
          _pagination = {
            'currentPage': 1,
            'totalPages': 1,
            'total': _statusCursos.length,
            'hasNextPage': false,
            'hasPrevPage': false,
          };
        });
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Erro ao buscar status de cursos: $e'),
            backgroundColor: Colors.red,
          ),
        );
      }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Gerenciar Status de Cursos'),
        actions: [
          IconButton(
          ...
          IconButton(
            ...
            tooltip: 'Adicionar Status de Curso',
          ),
        ],
        // bottom: TabBar(
  ...
  Widget _buildListaTab() {
    return Column(
      children: [
        ...
        Expanded(child: _buildStatusCursosList()),
            ...
            children: [
              Expanded(
                child: _buildStatCard(
                  'Total de Status',
                  (_pagination?['total'] ?? _statusCursos.length).toString(),
                  Icons.description,
              ...
              Expanded(
                child: _buildStatCard(
                  'Ativos',
                  (_statusCursos.where((s) => s.ativo).length).toString(),
                  Icons.check_circle,
              ...
              Expanded(
                child: _buildStatCard(
                  'Padrão',
                  (_statusCursos.where((s) => s.isDefault).length).toString(),
                  Icons.star,
        ...
        child: Column(
          ...
          children: [
              ...
              children: [
                Text(
                  _statusEditando == null
                      ? 'Novo Status de Curso'
                      : 'Editar Status de Curso',
            ...
            CustomTextField(
              ...
              label: 'Nome do Status de Curso *',
              maxLines: 1,
              validator: (value) =>
                  Validators.validateRequired(value, 'Nome do Status de Curso'),
            ),
            const SizedBox(height: 16),
            CheckboxListTile(
  ...
  Widget _buildStatusCursosList() {
          ...
          children: [
            ...
            Text(
              _currentSearch.isNotEmpty
                  ? 'Nenhum status de curso encontrado para a busca "$_currentSearch"'
                  : 'Nenhum status de curso cadastrado',
            ...
            if (_currentSearch.isNotEmpty)
              ...
              ElevatedButton(
                ...
                child: const Text('Adicionar Primeiro Status'),
              ),
          ],
        ),
      );
    ...
    return ListView.builder(
      padding: const EdgeInsets.all(16),
      itemCount: _statusCursos.length,
      itemBuilder: (context, index) {
        final statusCurso = _statusCursos[index];
        return _buildStatusCursoCard(statusCurso, index);
      },
    );
  }

  Widget _buildStatusCursoCard(StatusCurso statusCurso, int index) {
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Container(
                  ...
                  child: Center(
                    child: Text(
                      statusCurso.ordem?.toString() ?? '${index + 1}',
                      style: const TextStyle(
                        fontSize: 12,
                        fontWeight: FontWeight.bold,
                        color: Color(0xFF2E7D32),
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      if (statusCurso.descricao.isNotEmpty)
                        Text(
                          statusCurso.descricao,
                          style: TextStyle(
                            color: Colors.grey[600],
                            fontSize: 12,
                          ),
                ...
                PopupMenuButton<String>(
                  onSelected: (value) => _handleMenuAction(value, statusCurso),
                  itemBuilder: (context) => [
                    const PopupMenuItem(
                      value: 'editar',
                      child: Row(
                    ...
                    PopupMenuItem(
                      value: statusCurso.ativo ? 'desativar' : 'ativar',
                      child: Row(
                        children: [
                          Icon(
                            statusCurso.ativo
                                ...
                                : Icons.visibility,
                            size: 18,
                            color: statusCurso.ativo
                                ? Colors.orange
                                : Colors.green,
                          ),
                          const SizedBox(width: 8),
                          Text(
                            statusCurso.ativo ? 'Desativar' : 'Ativar',
                            style: TextStyle(
                              color: statusCurso.ativo
                                  ? Colors.orange
                                  : Colors.green,
            ...
            //         'Criado em',
            //         statusCurso.createdAt != null
            //             ? AppUtils.formatDate(statusCurso.createdAt!)
            //             : 'N/A',
            ...
            //       child: _buildInfoChip(
            //         'Usos',
            //         statusCurso.totalUsos.toString(),
            //         Icons.assignment,
            //       ),
            //     ),
            //   ],
              ...
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                      color: statusCurso.ativo ? Colors.green : Colors.grey,
                    ),
                  ),
                  child: Text(
                    statusCurso.ativo ? 'ATIVO' : 'INATIVO',
                    style: TextStyle(
                      fontSize: 10,
                      fontWeight: FontWeight.bold,
                      color: statusCurso.ativo ? Colors.green : Colors.grey,
  ...
  Widget _buildPaginationControls() {
    if (_statusCursos.isEmpty && !_isLoading && !_isLoadingPage) {
      return const SizedBox.shrink();
    }

    final totalPages = _pagination?['totalPages'] ??
        ((_statusCursos.isNotEmpty)
            ? ((_statusCursos.length / _pageSize)
                .ceil()
                .clamp(1, double.infinity)
                .toInt())
            : 1);
    final currentPage = _pagination?['currentPage'] ?? _currentPage;
    final total = _pagination?['total'] ?? _statusCursos.length;
    final hasNextPage =
  ...
  void _goToPage(int page) {
    if (page != _currentPage && page >= 1) {
      ...
      _loadStatusCursos(showLoading: false);
    }
  }
  ...
  void _handleMenuAction(String action, StatusCurso statusCurso) {
    switch (action) {
      case 'editar':
        _editarStatusCurso(statusCurso);
        break;
      case 'ativar':
        _ativarStatusCurso(statusCurso);
        break;
      case 'desativar':
        _desativarStatusCurso(statusCurso);
        break;
      case 'excluir':
        _confirmarExclusao(statusCurso);
        break;
    }
  }

  void _editarStatusCurso(StatusCurso statusCurso) {
    setState(() {
      _statusEditando = statusCurso;
      _nomeController.text = statusCurso.nome;
      //_descricaoController.text = statusCurso.descricao;
      _ordemController.text = statusCurso.ordem?.toString() ?? '';
      _corSelecionada = statusCurso.cor;
      _ativo = statusCurso.ativo;
      _isDefault = statusCurso.isDefault;
      _showForm = true;
    });
  }

  Future<void> _ativarStatusCurso(StatusCurso statusCurso) async {
    try {
      final success =
          await StatusCursoService.ativarStatusCurso(statusCurso.id!);
      if (success) {
        AppUtils.showSuccessSnackBar(
            context, 'Status de curso ativado com sucesso!');
        _loadStatusCursos(showLoading: false);
      }
    } catch (e) {
  ...
  Future<void> _desativarStatusCurso(StatusCurso statusCurso) async {
    try {
      final success =
          await StatusCursoService.desativarStatusCurso(statusCurso.id!);
      if (success) {
        AppUtils.showSuccessSnackBar(
            context, 'Status de curso desativado com sucesso!');
        _loadStatusCursos(showLoading: false);
      }
    } catch (e) {
      AppUtils.showErrorSnackBar(context, 'Erro: $e');
  ...
  Future<void> _confirmarExclusao(StatusCurso statusCurso) async {
    final confirm = await AppUtils.showConfirmDialog(
      context,
      title: 'Excluir Status de Curso',
      content:
          'Tem certeza que deseja excluir "${statusCurso.nome}"?\n\nEsta ação não pode ser desfeita e pode afetar cursos que utilizam este status.',
      confirmText: 'Excluir',
    );
    ...
    if (confirm) {
      try {
        final success =
            await StatusCursoService.deletarStatusCurso(statusCurso.id!);
        if (success) {
          AppUtils.showSuccessSnackBar(
              context, 'Status de curso excluído com sucesso!');
          _loadStatusCursos();
        }
      } catch (e) {
        AppUtils.showErrorSnackBar(context, 'Erro: $e');
  ...
  Future<void> _salvarStatus() async {
    ...
    try {
      ...
      if (_statusEditando == null) {
        success = await StatusCursoService.criarStatusCurso(dados);
      } else {
        success = await StatusCursoService.atualizarStatusCurso(
      ...
      if (success) {
        AppUtils.showSuccessSnackBar(
          context,
          _statusEditando == null
              ? 'Status de curso criado com sucesso!'
              : 'Status de curso atualizado com sucesso!',
        );
        _cancelarForm();
        _loadStatusCursos();
      }
  ...
  Future<void> _exportarDados() async {
    try {
      await StatusCursoService.exportarCSV(
        ativo: _filtroAtivo,
        isDefault: _filtroDefault,
      );
