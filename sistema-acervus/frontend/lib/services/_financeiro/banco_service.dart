// lib/services/banco_service.dart
import 'dart:convert';
import 'package:http/http.dart' as http;
import '../../models/_core/banco.dart';
...
class BancoService {
  ...
  static Future<Map<String, dynamic>> listarBancos({
    int page = 1,
    int limit = 10,
    String? search,
    bool? ativo,
    ...
    try {
      ...
      final uri = Uri.parse('$baseUrl/banco/listar')
  ...
  static Future<List<Banco>?> buscarBanco(String query) async {
    try {
      final uri = Uri.parse('$baseUrl/banco/listar')
  ...
  static Future<Banco> buscarBancoPorId(int id) async {
    try {
      final response = await http.get(
        Uri.parse('$baseUrl/banco/$id'),
        headers: await _getHeaders(),
      );

      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        return Banco.fromJson(data['dados']);
      } else {
  ...
  /// Cria um novo ítem
  static Future<bool> criarBanco(Map<String, dynamic> dados) async {
    try {
      final response = await http.post(
        Uri.parse('$baseUrl/banco/cadastrar'),
        headers: await _getHeaders(),
  ...
  static Future<bool> atualizarBanco(int id, Map<String, dynamic> dados) async {
    try {
      final response = await http.put(
        Uri.parse('$baseUrl/banco/alterar/$id'),
        headers: await _getHeaders(),
        body: jsonEncode(dados),
      );
  ...
  /// Ativa um ítem
  static Future<bool> ativarBanco(int id, {bool ativo = true}) async {
    try {
      final response = await http.put(
        Uri.parse('$baseUrl/banco/alterar/$id'),
        headers: await _getHeaders(),
  ...
  /// Desativa um ítem
  static Future<bool> desativarBanco(int id, {bool ativo = false}) async {
    try {
      final response = await http.put(
        Uri.parse('$baseUrl/banco/alterar/$id'),
        headers: await _getHeaders(),
  ...
  /// Exclui um ítem
  static Future<bool> deletarBanco(int id) async {
    try {
      final response = await http.delete(
        Uri.parse('$baseUrl/banco/excluir/$id'),
        headers: await _getHeaders(),
      );
  ...
  /// Reordena os ítems
  static Future<bool> reordenarBancos(List<Map<String, dynamic>> ordem) async {
    try {
      final response = await http.put(
        Uri.parse('$baseUrl/banco/reordenar'),
        headers: await _getHeaders(),
  ...
  static Future<Map<String, dynamic>> getEstatisticasGerais() async {
    try {
      final response = await http.get(
        Uri.parse('$baseUrl/banco/estatisticas'),
        headers: await _getHeaders(),
      );
  ...
  static Future<Banco?> duplicarBanco(int id) async {
    try {
      final response = await http.post(
        Uri.parse('$baseUrl/banco/$id/duplicar'),
        headers: await _getHeaders(),
      );

      if (response.statusCode == 201) {
        final data = jsonDecode(response.body);
        return Banco.fromJson(data['data']);
      } else {
  ...
  static Future<Map<String, dynamic>> getNiveisBancoComStatus(
    int statusId, {
    int page = 1,
    int limit = 10,
  }) async {
    try {
      ...
      final uri = Uri.parse('$baseUrl/banco/$statusId/items')
          .replace(queryParameters: queryParams);
      final response = await http.get(uri, headers: await _getHeaders());
  ...
  static Future<String?> exportarCSV({
    ...
    try {
      ...
      final uri = Uri.parse('$baseUrl/banco/exportar/csv')
          .replace(queryParameters: queryParams);
      final response = await http.get(uri, headers: await _getHeaders());
      ...
      if (response.statusCode == 200) {
        ...
        final filename = _extrairFilename(
            ...
            'banco_${DateTime.now().toIso8601String().substring(0, 10)}.csv';
  ...
  static Future<List<int>> exportarNiveisBancoPDF({
    bool? ativo,
    bool? isDefault,
  }) async {
    try {
      ...
      final uri = Uri.parse('$baseUrl/banco/exportar/pdf')
          .replace(queryParameters: queryParams);
      final response = await http.get(uri, headers: await _getHeaders());
  ...
  static Future<List<String>> getCachedCoresDisponiveis() async {
    const cacheKey = 'cores_disponiveis_niveis_banco';
  ...
  static Future<Map<String, dynamic>> getCachedEstatisticasGerais() async {
    const cacheKey = 'estatisticas_gerais_niveis_banco';
    final cached = _getCache(cacheKey);
  ...
  static Future<List<Banco>> getCachedStatusAtivos() async {
    const cacheKey = 'status_niveis_banco_ativos';
    final cached = _getCache(cacheKey);

    if (cached != null) {
      return cached.map<Banco>((json) => Banco.fromJson(json)).toList();
    }

    final result = await listarBancos(ativo: true, limit: 100);
    final statusList = result['bancos'] as List<Banco>;
